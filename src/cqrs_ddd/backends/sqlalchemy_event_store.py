...
class SQLAlchemyEventStore:
    ...
    def __init__(
        ...
        try:
            from sqlalchemy.ext.asyncio import AsyncSession as _AsyncSession, async_sessionmaker as _AsyncSessionMaker  # type: ignore
        except Exception:
            raise ImportError("SQLAlchemy is required")
    ...
    async def append(
        ...
        try:
            ...
            if not hasattr(uow, 'session'):
                raise TypeError("SQLAlchemyEventStore requires a UnitOfWork with a 'session' attribute")
    ...
    async def get_current_version(
        ...
        if unit_of_work:
            return await self._get_version(unit_of_work.session, aggregate_type, aggregate_id)
        else:
            async with self._uow_factory() as uow:
                return await self._get_version(uow.session, aggregate_type, aggregate_id)

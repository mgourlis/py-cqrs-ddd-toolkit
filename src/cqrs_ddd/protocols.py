...
class LockStrategy(Protocol):
    ...
    async def acquire(
        self, entity_type: str, entity_id_or_ids: Any, timeout: int = 30
    ) -> str:
        """
    ...
    async def release(
        self, entity_type: str, entity_id_or_ids: Any, token: str
    ) -> None:
        """
...
class MessageConsumer(Protocol):
    ...
    async def subscribe(
        self, topic: str, handler: Any, queue_name: Optional[str] = None
    ) -> None:
        """
...
class EventStore(Protocol):
    ...
    async def append(self, event: Any, expected_version: Optional[int] = None) -> Any:
        """Append an event to the store."""
        ...

    async def append_batch(
        self, events: List[Any], correlation_id: Optional[str] = None
    ) -> List[Any]:
    ...
    async def get_events_by_correlation(self, correlation_id: str) -> List[Any]:
...
class UndoService(Protocol):
    ...
    async def get_undo_stack(
        self, aggregate_type: str, aggregate_id: Any, depth: int = 10
    ) -> List[Any]:

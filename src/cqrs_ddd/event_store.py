...
# Import domain event generators for middleware usage
from .domain_event import generate_event_id, generate_correlation_id
...
class InMemoryEventStore(EventStore):
    ...
    def __init__(self):
        ...
        self._versions: Dict[tuple, int] = {}  # (aggregate_type, aggregate_id) -> version
        self._next_id = 1
    ...
    async def get_events(
        ...
        logger.debug(f"EventStore.get_events: Found {len(results)} events for {aggregate_type}:{aggregate_id}")
        if len(self._events) > 0 and len(results) == 0:
             # Log potential mismatches for debugging
             for e in self._events:
                 logger.debug(f"Event in store: {e.aggregate_type}:{e.aggregate_id} (ID: {e.id}, Type: {e.event_type})")
    ...
    async def get_undone_events(
        ...
        results = sorted(results, key=lambda e: e.undone_at or e.occurred_at, reverse=True)

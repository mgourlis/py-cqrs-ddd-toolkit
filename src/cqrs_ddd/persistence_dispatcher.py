...
from typing import (
    Type, Dict, Callable, Union, List, Any, Optional, 
    TypeVar, Generic, get_args, get_origin, Tuple, TYPE_CHECKING
)
import logging
...
def _register_handler(registry: Dict, key: Type, handler_cls: Type, priority: int):
    ...
    logger.debug(f"Registered {handler_cls.__name__} for {key.__name__} (priority={priority})")
...
def _extract_generic_args(cls: Type, base_generic: Type) -> List[Type]:
    ...
    for base in getattr(cls, '__orig_bases__', []):
        ...
        if origin is base_generic or (isinstance(origin, type) and issubclass(origin, base_generic)):
            args = get_args(base)
...
def list_modification_handlers() -> Dict[str, List[str]]:
    return {k.__name__: [e.handler_cls.__name__ for e in v] for k, v in _modification_handlers.items()}

def list_retrieval_handlers() -> Dict[str, List[str]]:
    return {k.__name__: [e.handler_cls.__name__ for e in v] for k, v in _retrieval_handlers.items()}

def list_query_handlers() -> Dict[str, List[str]]:
    return {k.__name__: [e.handler_cls.__name__ for e in v] for k, v in _query_handlers.items()}
...
class OperationPersistence(ABC, Generic[T_Mod]):
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        for mod_type in mod_types:
            _register_handler(_modification_handlers, mod_type, cls, getattr(cls, 'priority', 0))
        
        # Diagnostic: If no types found and not an intermediate generic class
        if not mod_types and not any(isinstance(a, TypeVar) for b in getattr(cls, '__orig_bases__', []) for a in get_args(b)):
             logger.warning(f"Class {cls.__name__} inherits from OperationPersistence but specifies no modification types.")
    
    @abstractmethod
    async def persist(self, modification: T_Mod, unit_of_work: Any) -> Union[Any, List[Any]]:
        """
...
class RetrievalPersistence(ABC, Generic[T_Entity]):
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        for entity_type in entity_types:
            _register_handler(_retrieval_handlers, entity_type, cls, getattr(cls, 'priority', 0))
            
        # Diagnostic
        if not entity_types and not any(isinstance(a, TypeVar) for b in getattr(cls, '__orig_bases__', []) for a in get_args(b)):
             logger.warning(f"Class {cls.__name__} inherits from RetrievalPersistence but specifies no entity types.")

    @abstractmethod
    async def retrieve(self, entity_ids: List[Any], unit_of_work: Any) -> List[T_Entity]:
...
class QueryPersistence(ABC, Generic[T_Result]):
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        for result_type in result_types:
            _register_handler(_query_handlers, result_type, cls, getattr(cls, 'priority', 0))
...
class PersistenceDispatcher:
    ...
    async def apply(
        ...
        should_cache = all(getattr(self._handler_factory(h), 'use_cache', True) for h in handler_classes)
    ...
    async def fetch_domain(
        ...
        if not handler_classes:
            raise ValueError(f"No retrieval handler registered for {entity_type.__name__}")
        
        handler = self._handler_factory(handler_classes[0])

        # Read-Through Cache (Domain)
        if self._cache_service and handler.entity_name and getattr(handler, 'use_cache', True):

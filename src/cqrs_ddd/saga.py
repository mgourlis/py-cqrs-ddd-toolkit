...
from typing import Any, Dict, List, Optional, Type, TypeVar, Generic, TYPE_CHECKING, Union, get_origin, get_args
...
try:
    from tenacity import retry, retry_if_exception_type, stop_after_attempt, wait_exponential, before_sleep_log, RetryError
    HAS_TENACITY = True
except ImportError:
    HAS_TENACITY = False
...
class Saga(ABC, Generic[TState]):
        ...
        if state_type and dataclasses.is_dataclass(state_type):
             ...
             if self.context.state:
                 try:
                     ...
                     logger.error(f"Failed to parse existing state into dataclass {state_type}: {e}")
                     raise ValueError(f"Saga State Corruption: Could not bind context to {state_type.__name__}") from e
             else:
                 try:
    ...
    def _serialize_command(self, command: Command) -> Dict[str, Any]:
        ...
        data = asdict(command) if dataclasses.is_dataclass(command) else command.__dict__
        return {
...
class InMemorySagaRepository(SagaRepository):
    ...
    async def find_expired_suspended_sagas(self, limit: int = 10) -> List[SagaContext]:
        ...
        for ctx in self._sagas.values():
            if ctx.is_suspended and ctx.suspend_timeout_at and ctx.suspend_timeout_at <= now:
                expired.append(ctx)
...
class SagaManager(ABC):
    ...
    async def _dispatch_and_record_command(self, command: Command, context: SagaContext) -> None:
        """
    ...
    async def recover_pending_sagas(self, limit: int = 10):
        ...
        if not hasattr(self.repository, 'find_stalled_sagas'):
            logger.warning("SagaRepository does not support find_stalled_sagas, skipping recovery.")
            return

        stalled_contexts = await self.repository.find_stalled_sagas(limit)
        
        for context in stalled_contexts:
            logger.info(f"Recovering stalled saga {context.saga_id} with {len(context.pending_commands)} pending commands.")
                ...
                for cmd_data in cmds_to_dispatch:
                    ...
                    try:
                        ...
                        if module_name and type_name:
                            ...
                            logger.error(f"Invalid serialized command in Saga {context.saga_id}: {cmd_data}")
                    except Exception as e:
                        logger.error(f"Failed to recover command in Saga {context.saga_id}: {e}")
    ...
    async def process_timeouts(self, limit: int = 10):
        ...
        for context in expired_contexts:
            logger.info(f"Processing timeout for Saga {context.saga_id} (Reason: {context.suspend_reason})")
            ...
            if self.lock_strategy:
                ...
                try:
                    ...
                    logger.warning(f"Could not acquire lock for timeout processing {context.saga_id}: {e}")
                    continue
            ...
            try:
                ...
                if not saga_class:
                    logger.error(f"Could not find Saga class {context.saga_type} for timeout processing")
                    continue
                ...
                if context.is_suspended and context.suspend_timeout_at and context.suspend_timeout_at <= now:
                     # Avoid infinite loop
                     logger.warning(f"Saga {context.saga_id} on_timeout did not resolve suspension. Forcing failure.")
                ...
                logger.error(f"Failed to process timeout for saga {context.saga_id}: {e}")
            finally:
    ...
    async def _dispatch_pending_commands(self, context: SagaContext):
         ...
         for cmd_data in cmds_to_dispatch:
             try:
                 ...
                 logger.error(f"Failed to dispatch pending command for saga {context.saga_id}: {e}")
    ...
    async def _process_saga(self, saga_class: Type['Saga'], correlation_id: str, event: Optional[DomainEvent] = None, input_data: Any = None) -> Any:
        try:
            lock_token = None
            lock_token = None
            if self.lock_strategy:
            ...
            try:
                ...
                try:
                    for cmd in saga._local_pending_commands:
                     ...
                     logger.error(f"Saga {context.saga_id} stalled during dispatch: {dispatch_err}")
...
class SagaOrchestratorManager(SagaManager):
    ...
    """
    async def run(self, saga_class: Type['Saga'], input_data: Any, correlation_id: str) -> str:
        # If input data is a DomainEvent (or Command), treat it as an event handling step
        if isinstance(input_data, DomainEvent):
        ...
        return await self._process_saga(saga_class, correlation_id, event=None, input_data=input_data)

...
class EventDispatcher:
    ...
    def _discover_handlers(self, event_type: Type) -> None:
        ...
        for handler_cls in handlers_meta.get('priority', []):
            ...
            if not any(isinstance(h, handler_cls) for h in self._priority_subscribers.get(event_type, [])):
                try:
                    self.register(event_type, handler_cls(), priority=True)
                except Exception as e:
                    logger.error(f"Failed to instantiate priority handler {handler_cls.__name__}: {e}")
                    
        # Register background handlers
        for handler_cls in handlers_meta.get('background', []):
            if not any(isinstance(h, handler_cls) for h in self._subscribers.get(event_type, [])):
                try:
                    self.register(event_type, handler_cls(), priority=False)
                except Exception as e:
                    logger.error(f"Failed to instantiate background handler {handler_cls.__name__}: {e}")

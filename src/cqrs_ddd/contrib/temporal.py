...
class TemporalSagaChoreographyManager(SagaChoreographyManager):
    ...
    async def handle_event(self, event: DomainEvent) -> None:
        ...
        if not correlation_id:
             logger.warning(f"Event {event_type.__name__} missing correlation_id, skipping Temporal signal.")
             return
...
class TemporalSagaOrchestratorManager(SagaOrchestratorManager):
    ...
    async def run(self, saga_class: Type[Saga], input_data: Any, correlation_id: str) -> str:
        ...
        try:
            ...
            logger.info(f"Started Temporal Orchestration {workflow_name} (ID: {workflow_id})")
            return handle.id

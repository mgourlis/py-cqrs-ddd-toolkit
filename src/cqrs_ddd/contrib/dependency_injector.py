...
from ..protocols import EventStore, CacheService, LockStrategy, UndoExecutorRegistry
...
from ..saga import InMemorySagaRepository
...
def autowire_router_startup(dispatcher: EventDispatcher, publisher: Any, packages: Optional[list] = None):
    if packages:
...
class Container(containers.DeclarativeContainer):
    ...
    if HAS_DRAMATIQ:
        ...
        dramatiq_routing_actor = providers.Object(dramatiq_tasks.default_domain_event_router)
    ...
    # Outbox Storage
    outbox_storage = providers.Singleton("cqrs_ddd.backends.sqlalchemy_outbox.SQLAlchemyOutboxStorage")
    ...
    outbox_publisher = providers.Factory(
        "cqrs_ddd.outbox.OutboxPublisher",
...
class WiredEventStorePersistenceMiddleware(EventStorePersistenceMiddleware):
    ...
    def __init__(
        self, 
        event_store: EventStore = Provide[Container.event_store]
    ):

...
class TracingBackend(Protocol):
    ...
    def start_span(self, name: str, kind: Any = None, attributes: Dict[str, Any] = None):
...
class SentryBackend(TracingBackend):
    ...
    def start_span(self, name: str, kind: Any = None, attributes: Dict[str, Any] = None):
        ...
        op = attributes.get("messaging.operation", "unknown") if attributes else "unknown"
...
def configure_tracing(backend_name: str = "noop", service_name: str = "cqrs-ddd", custom_backend: TracingBackend = None):
    """
...
def traced_persistence(cls):
    ...
    methods = [
        'persist', 
        'retrieve', 'fetch_domain', 'fetch_domain_one',
        'fetch', 'fetch_one',
        'retrieve_batch', 'retrieve_one', 'try_retrieve'
    ]
...
def instrument_dispatcher(persistence_dispatcher: Any):
    ...
    methods = [
        'apply',
        'fetch', 'fetch_one',
        'fetch_domain', 'fetch_domain_one',
        'retrieve', 'retrieve_one'
    ]
    ...
    for method_name in methods:
        if hasattr(persistence_dispatcher, method_name):
            ...
            setattr(persistence_dispatcher, method_name, trace_span(span_name)(original))
...
def traced_saga(cls):
    ...
    if hasattr(cls, 'compensate'):
        ...
        setattr(cls, 'compensate', trace_span(f"saga:{cls.__name__}.compensate")(original))

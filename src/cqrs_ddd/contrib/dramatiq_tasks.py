...
class DramatiqEventPublisher(MessagePublisher):
    ...
    async def publish(self, topic: str, message: Any, **kwargs: Any) -> None:
        ...
        wrapped_message = {"topic": topic, "payload": payload, "metadata": kwargs}

        # Send to Dramatiq
        # .send() is synchronous, so we run it in an executor to avoid blocking the event loop
        loop = asyncio.get_running_loop()
        await loop.run_in_executor(
            None, lambda: self.routing_actor.send(wrapped_message)
        )
        logger.debug(
            f"Published event '{topic}' to Dramatiq actor {self.routing_actor.actor_name}"
        )
...
async def handle_dramatiq_event(message: Dict[str, Any], dispatcher: Any) -> None:
    """

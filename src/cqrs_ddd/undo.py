...
class UndoExecutor(UndoExecutorProtocol[T], ABC):
    ...
    def __init__(
        self, persistence_dispatcher: PersistenceDispatcher = None, mediator: Any = None
    ):
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if et:
            ...
            logger.warning(
                f"Could not determine event type for UndoExecutor {cls.__name__}"
            )
...
class DefaultUndoService(UndoService):
    ...
    async def _invalidate_cache(self, aggregate_type: str, aggregate_id: Any) -> None:
        """
    ...
    async def redo(
        ...
        for undo_event in events_to_redo_from:
            ...
            if not original_event_id:
                errors.append(
                    f"Cannot determine original event for undo event {undo_event.event_id}"
                )
                continue

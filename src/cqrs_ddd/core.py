...
if TYPE_CHECKING:
    from .persistence import PersistenceDispatcher
...
class Command(AbstractCommand):
    ...
    _command_id: str = field(default_factory=lambda: str(import_uuid().uuid4()), repr=False)
...
class Query(AbstractQuery):
    ...
    _query_id: str = field(default_factory=lambda: str(import_uuid().uuid4()), repr=False)
    _correlation_id: str = field(default=None, repr=False)
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if hasattr(cls, 'handle'):
            try:
                ...
                if len(params) >= 2:
                    cmd_param = params[1]
                    if cmd_param.annotation is not inspect.Parameter.empty and isinstance(cmd_param.annotation, type):
                        ...
                        if issubclass(cmd_param.annotation, AbstractCommand):
                            ...
                            logger.warning(f"Could not register {cls.__name__}: {cmd_param.annotation.__name__} does not inherit from AbstractCommand")
                    else:
                        logger.warning(f"Could not register {cls.__name__}: First argument of handle() must have a type annotation")
                else:
                    logger.warning(f"Could not register {cls.__name__}: handle() method must accept at least one argument (besides self)")
        ...
        if hasattr(cls, 'handle'):
            try:
                ...
                if len(params) >= 2:
                    ...
                    if query_param.annotation is not inspect.Parameter.empty and isinstance(query_param.annotation, type):
                        ...
                        if issubclass(query_param.annotation, AbstractQuery):
                            ...
                            logger.warning(f"Could not register {cls.__name__}: {query_param.annotation.__name__} does not inherit from AbstractQuery")
                    else:
                        logger.warning(f"Could not register {cls.__name__}: First argument of handle() must have a type annotation")
                else:
                    logger.warning(f"Could not register {cls.__name__}: handle() method must accept at least one argument (besides self)")
                        ...
                        if origin in (Union, UnionType):
                            ...
                            for et in event_types:
                                if et is not type(None):
                                    register_event_handler(et, cls, getattr(cls, 'is_priority', False))
                            # For internal tracking, store the first one or a tuple
                            cls._handles_event = event_types[0]
                        elif isinstance(annotation, type):
                            # Handle single Event
                            register_event_handler(annotation, cls, getattr(cls, 'is_priority', False))
                            cls._handles_event = annotation
                        else:
                            logger.warning(f"Could not register {cls.__name__}: Invalid type annotation {annotation}")
                    else:
                        logger.warning(f"Could not register {cls.__name__}: First argument of handle() must have a type annotation")
                else:
                    logger.warning(f"Could not register {cls.__name__}: handle() method must accept at least one argument (besides self)")

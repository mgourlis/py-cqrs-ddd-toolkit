...
from .core import Command, Query, CommandResponse, AbstractCommand, AbstractQuery
...
class Mediator:
    ...
    def set_container(self, container: Any) -> None:
        ...
        if not self._event_dispatcher:
            ...
            if hasattr(core, 'event_dispatcher'):
                ...
                logger.warning("Mediator: No event_dispatcher set, background events will NOT be dispatched")
    ...
    async def _dispatch_with_middlewares(
        ...
        for middleware_item in middlewares:
            ...
            if isinstance(middleware_item, MiddlewareDefinition):
                ...
                try:
                    middleware_instance = middleware_item.middleware_class(**middleware_item.kwargs)
                except Exception as e:
                    logger.error(f"Failed to instantiate middleware {middleware_item.middleware_class}: {e}")
                    raise
    ...
    def _get_handler(self, message_type: Type) -> object:
        ...
        if not handler:
            ...
            handler_cls = registered['commands'].get(message_type) or registered['queries'].get(message_type)
            ...
            if handler_cls:
                try:
                    ...
                    logger.debug(f"Direct instantiation of {handler_cls.__name__} failed (expected if it has dependencies): {e}")
        ...
        if callable(handler) and not isinstance(handler, type) and not hasattr(handler, 'handle'):

...
def register_command_handler(command_type: Type, handler_class: Type) -> None:
    ...
    if not command_type:
        ...
        logger.warning(
            f"Failed to register command handler for {name}: command_type is None"
        )
        return
    _command_handlers[command_type] = handler_class
    logger.debug(
        f"Registered CommandHandler {handler_class.__name__} for {command_type.__name__}"
    )
...
def register_query_handler(query_type: Type, handler_class: Type) -> None:
    ...
    if not query_type:
        name = handler_class.__name__ if handler_class else "Unknown"
        logger.warning(
            f"Failed to register query handler for {name}: query_type is None"
        )
        return
    _query_handlers[query_type] = handler_class
    logger.debug(
        f"Registered QueryHandler {handler_class.__name__} for {query_type.__name__}"
    )


def register_event_handler(
    event_type: Type, handler_class: Type, priority: bool = False
) -> None:
    """
    ...
    if not event_type:
        name = handler_class.__name__ if handler_class else "Unknown"
        logger.warning(
            f"Failed to register event handler for {name}: event_type is None"
        )
        return
    ...
    logger.debug(
        f"Registered EventHandler {handler_class.__name__} for {event_type.__name__} ({type_kind})"
    )

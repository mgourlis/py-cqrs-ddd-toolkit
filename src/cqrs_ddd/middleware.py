...
from typing import Type, Callable, Any, Union, Dict, Optional

from .protocols import Middleware, AttributeInjector, EventStore, LockStrategy
import logging
...
class LoggingMiddleware(Middleware):
    ...
    def apply(self, handler_func: Callable, message) -> Callable:
        ...
        async def wrapped(*args, **kwargs):
            ...
            if not trace_id:
                trace_id = getattr(message, 'command_id', None) or getattr(message, 'query_id', None)
            ...
            try:
                ...
                logger.info(f"{id_prefix}Completed {command_name} in {duration.total_seconds():.3f}s")
                return result
...
# =============================================================================
# Thread Safety / Concurrency Middleware
# =============================================================================
...
class ThreadSafetyMiddleware(Middleware):
    ...
    def apply(self, handler_func, command):
        ...
        async def wrapped_handler(*args, **kwargs):
            ...
            if not strategy:
                logger.warning("ThreadSafetyMiddleware: No lock strategy configured, skipping lock")
                return await handler_func(*args, **kwargs)
            
            # Get entity ID(s)
            entity_id_or_ids = getattr(command, self.entity_id_attr, None)
            if entity_id_or_ids is None:
                raise ValueError(f"Command must contain '{self.entity_id_attr}' attribute")
            
            token = None
            try:
                logger.debug(f"ThreadSafety: Acquiring lock for {self.entity_type}:{entity_id_or_ids}")
                token = await strategy.acquire(self.entity_type, entity_id_or_ids)
                logger.debug(f"ThreadSafety: Lock acquired for {self.entity_type}:{entity_id_or_ids}")
                return await handler_func(*args, **kwargs)
            finally:
                if token:
                    logger.debug(f"ThreadSafety: Releasing lock for {self.entity_type}:{entity_id_or_ids}")
    ...
    def apply(self, handler_func, command):
        ...
        async def wrapped_handler(*args, **kwargs):
            ...
            if hasattr(result, 'correlation_id') and not result.correlation_id:
                try: result.correlation_id = correlation_id
                except (AttributeError, TypeError): pass
            if hasattr(result, 'causation_id') and not result.causation_id:
                try: result.causation_id = causation_id
                except (AttributeError, TypeError): pass
            ...
            if events:
                ...
                if not event_store:
                    logger.warning("EventStorePersistenceMiddleware: No event store provided, skipping persistence")
                    return result
                
                from .domain_event import enrich_event_metadata
                enriched_events = [
                    enrich_event_metadata(event, correlation_id=correlation_id, causation_id=causation_id)
                    for event in events
                ]
                
                try:
                    logger.debug(f"EventStorePersistenceMiddleware: Appending batch of {len(enriched_events)} events")
                    await event_store.append_batch(enriched_events, correlation_id=correlation_id)
                    logger.debug(f"Persisted {len(enriched_events)} events with correlation_id={correlation_id}")
                except Exception as e:
...
class MiddlewareRegistry:
    ...
    def _register(self, handler_class, middleware_class: Type['Middleware'], **kwargs):
        ...
        handler_class._middlewares.append(MiddlewareDefinition(
    ...
    def inject_attrs(self, injectors: dict):
        ...
        def decorator(handler_class):
            return self._register(
                handler_class, 
                self.classes['attribute_injection'], 
                injectors=injectors
            )
        return decorator

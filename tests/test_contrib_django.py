...
async def test_cqrs_view_dispatch():
    ...
    my_container.core = (
        my_container  # For getattr(self.container, 'core', self.container)
    )
...
def test_exception_middleware():
    ...
    resp = middleware.process_exception(
        req, ValidationError(errors={"field": ["err"]}, message_type="Cmd")
    )
    ...
    # Not Found
    resp = middleware.process_exception(
        req, EntityNotFoundError("Not found", entity_id=123)
    )
    ...
    # Infra
    resp = middleware.process_exception(
        req, InfrastructureError(code="INFRA_ERROR", message="Down")
    )

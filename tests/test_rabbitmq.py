...
async def test_subscribe_declares_queue_and_consumes(monkeypatch, caplog):
    ...
    channel_mock = SimpleNamespace(
        declare_queue=declare_queue, declare_exchange=declare_exchange
    )
    ...
    fake_aio = SimpleNamespace(
        connect_robust=connect_robust, ExchangeType=SimpleNamespace(TOPIC="topic")
    )
    ...
    assert any(
        "Error processing message" in rec.message
        for rec in caplog.records
        if rec.levelname == "ERROR"
    )
    assert any(
        "Subscribed to" in rec.message
        for rec in caplog.records

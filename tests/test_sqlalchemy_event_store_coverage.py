...
class DummyEvent:
    ...
    def to_dict(self):
        return {"msg": self.payload["msg"], "event_id": self.event_id}
...
async def test_event_store_batch_and_correlation(uow_factory):
    es = SQLAlchemyEventStore(uow_factory=uow_factory)
    events = [DummyEvent(aggregate_id="agg3"), DummyEvent(aggregate_id="agg3")]
...
async def test_event_store_errors(uow_factory):
    _ = SQLAlchemyEventStore(uow_factory=uow_factory)
    ...
    with pytest.raises(
        TypeError, match="requires a UnitOfWork with a 'session' attribute"
    ):

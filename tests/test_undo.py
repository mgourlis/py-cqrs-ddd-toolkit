...
class MockUndoExecutor(UndoExecutor[UndoTestEvent]):
    ...
    async def redo(self, original: UndoTestEvent, version1: UndoTestEvent) -> List[DomainEventBase]:
...
async def test_get_undo_stack_filtering():
    ...
    with patch("cqrs_ddd.undo.EventTypeRegistry.hydrate") as mock_hydrate:
        mock_hydrate.side_effect = lambda se: UndoTestEvent(data="valid") if se == e3 else None
...
async def test_undo_operation_success():
    ...
    with patch("cqrs_ddd.undo.EventTypeRegistry.hydrate") as mock_hydrate:
        ...
        mock_store.mark_as_undone.assert_called_once_with(
            event_id="evt-1", 
            undone_by="user-1", 
            undo_event_id=result.new_events[0]
        )
...
async def test_redo_operation_success():
    ...
    async def get_event_side_effect(eid):
        if eid == "undo-1": return undo_event
        if eid == "original-1": return original_event
        return None
    ...
    with patch("cqrs_ddd.undo.EventTypeRegistry.hydrate") as mock_hydrate:
        mock_hydrate.side_effect = [UndoTestEvent(data="orig"), UndoTestEvent(data="undo")]
...
async def test_undo_by_correlation():
    ...
    e1 = MagicMock(spec=StoredEvent); e1.id=1; e1.event_id="e1"; e1.is_undone=False; e1.event_type="UndoTestEvent"; e1.aggregate_type="Agg"; e1.aggregate_id="1"
    e2 = MagicMock(spec=StoredEvent); e2.id=2; e2.event_id="e2"; e2.is_undone=False; e2.event_type="UndoTestEvent"; e2.aggregate_type="Agg"; e2.aggregate_id="1"
...
async def test_redo_errors():
    ...
    undo_evt = MagicMock(spec=StoredEvent); undo_evt.causation_id = "orig"
    
    async def get_event_se(eid):
        if eid == "undo": return undo_evt
        return None

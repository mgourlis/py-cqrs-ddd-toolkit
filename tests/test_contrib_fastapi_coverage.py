...
def test_cqrs_router_init_variations():
    ...
    with pytest.raises(
        ValueError, match="requires either 'container' or 'mediator_provider'"
    ):
...
def test_cqrs_router_endpoints_coverage():
    ...
    # Command with mapper
    router.command("/cmd-mapped", MockCmd, response_mapper=lambda r: {"mapped": r})

    # Command without mapper
    router.command("/cmd", MockCmd)

    # Query with mapper
    router.query("/query-mapped", MockQuery, response_mapper=lambda r: {"mapped": r})

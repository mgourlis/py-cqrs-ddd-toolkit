...
async def test_manager_handle_event_missing_correlation_warning(caplog):
    ...
    manager = SagaChoreographyManager(MagicMock(), MagicMock(), saga_registry=saga_registry)
    event = MagicMock()
...
async def test_manager_process_saga_lock_coverage():
    ...
    repo = InMemorySagaRepository()
    manager = SagaChoreographyManager(repo, MagicMock(), lock_strategy=lock, saga_registry=saga_registry)
...
async def test_find_stalled_sagas_coverage():
    ...
    for i in range(3):
        ctx = SagaContext(saga_id=f"stalled-{i}", saga_type="T", correlation_id=f"c-{i}", current_step="S")
        ctx.is_stalled = True
        await repo.save(ctx)
...
async def test_recover_pending_sagas_coverage():
    ...
    ctx = SagaContext(saga_id="rec-1", saga_type="T", correlation_id="rec-c", current_step="S")
    ctx.is_stalled = True
...
async def test_recover_pending_sagas_failure_coverage(caplog):
    ...
    ctx = SagaContext(saga_id="fail-rec", saga_type="T", correlation_id="c", current_step="S")
    ctx.is_stalled = True
...
async def test_load_state_validation_error_coverage():
    ...
    with pytest.raises(ValueError, match="Saga State Corruption"):
        saga = BadStateSaga(ctx, MagicMock())

...
def test_wired_middleware_instantiation():
    ...
    with container.lock_strategy.override(MagicMock()):
        with container.event_store.override(MagicMock()):
            # Enforce wiring for the test
            container.wire(modules=[__name__])

            ts_m = WiredThreadSafetyMiddleware(entity_type="test", entity_id_attr="id")

...
if not HAS_TEMPORAL:
    pytest.skip(
        "skipping temporal tests because temporalio is not installed",
...
async def test_temporal_choreography_manager():
    ...
    manager = TemporalSagaChoreographyManager(
        client=client, task_queue="test-queue", saga_registry=registry
    )
...
async def test_temporal_orchestrator_manager():
    ...
    manager = TemporalSagaOrchestratorManager(
        client, task_queue="q2", saga_registry=saga_registry
    )

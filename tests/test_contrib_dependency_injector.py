...
def test_container_initialization():
    ...
    container.outbox_storage.override(MagicMock()) # Mock it to avoid init issues with real class
...
def test_install_hook():
    ...
    try:
        install()
        assert middleware_registry.classes['thread_safety'] == WiredThreadSafetyMiddleware
        assert middleware_registry.classes['event_store'] == WiredEventStorePersistenceMiddleware
    finally:
        # Restore
        if orig_ts:

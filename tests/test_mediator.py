...
from cqrs_ddd.mediator import Mediator, get_current_uow, _current_uow
...
class MyCommandHandler(CommandHandler[str]):
    async def handle(self, command: MyCommand) -> CommandResponse[str]:
        return CommandResponse(result=f"handled {command.data}", events=[MyEvent("event1")])
...
async def test_lazy_lookup_from_registry(mediator):
    ...
    with patch("cqrs_ddd.handler_registry.get_registered_handlers") as mock_get_registry:
...
async def test_nested_command_reuses_uow(mock_uow_factory):
    ...
    class OuterCommand(Command): pass
    class InnerCommand(Command): pass
...
async def test_middleware_execution():
    ...
    mock_mw_instance.apply = MagicMock(side_effect=lambda func, msg: func) # Identity wrapper
...
async def test_container_integration():
    ...
    container.event_dispatcher = None # Ensure it falls back to core or handle both logic provided in mediator

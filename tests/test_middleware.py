...
async def test_logging_middleware_execution():
    ...
    with patch("cqrs_ddd.middleware.logging.getLogger") as mock_get_logger:
...
async def test_thread_safety_missing_attr():
    middleware = ThreadSafetyMiddleware(
        entity_type="User",
        entity_id_attr="missing_field",
        lock_strategy=MagicMock()
    )
    command = SimpleCommand()
    handler = AsyncMock()
...
async def test_event_persistence_flow():
    ...
    with patch("cqrs_ddd.event_store.generate_correlation_id", return_value="gen-corr-id"):
        # We also need to patch enrich_event_metadata to return something we can assert on
        with patch("cqrs_ddd.domain_event.enrich_event_metadata", side_effect=lambda e, **k: e):
...
async def test_thread_safety_legacy_callable_strategy():
    ...
    strategy_factory = lambda: mock_lock
    
    middleware = ThreadSafetyMiddleware(
        entity_type="T", entity_id_attr="name", lock_strategy=strategy_factory
    )
    wrapped = middleware.apply(AsyncMock(return_value=CommandResponse(result="ok")), SimpleCommand(name="a"))
    await wrapped()
...
async def test_thread_safety_no_strategy():
    ...
    with patch("logging.getLogger") as mock_get_log:
        ...
        wrapped = middleware.apply(AsyncMock(return_value=CommandResponse(result="ok")), SimpleCommand())
...
async def test_event_store_persistence_error_handling():
    ...
    with patch("logging.getLogger") as mock_get_log:
        ...
        wrapped = middleware.apply(handler, SimpleCommand())
        await wrapped()
        mock_logger.warning.assert_called_with("EventStorePersistenceMiddleware: No event store provided, skipping persistence")

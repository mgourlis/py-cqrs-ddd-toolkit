...
class SimpleExecutor(UndoExecutor[CoverageTestEvent]):
    async def undo(self, event): return []
    async def redo(self, original, undo): return []
...
async def test_get_undo_stack_edge_cases():
    ...
    e1 = MagicMock(); e1.event_type="T1"; e1.is_undone=False; e1.id=1
    e2 = MagicMock(); e2.event_type="T2"; e2.is_undone=False; e2.id=2
    ...
    with patch("cqrs_ddd.undo.EventTypeRegistry.hydrate", return_value=CoverageTestEvent()):
...
async def test_get_undo_stack_depth_break():
    ...
    e1 = MagicMock(); e1.event_type="T1"; e1.is_undone=False; e1.event_id="e1"; e1.correlation_id="c1"; e1.id=1
    e2 = MagicMock(); e2.event_type="T1"; e2.is_undone=False; e2.event_id="e2"; e2.correlation_id="c2"; e2.id=2
...
async def test_undo_loop_branches():
    ...
    e_undone = MagicMock(); e_undone.is_undone = True; e_undone.id=1 # Hit 364
    
    e_no_exec = MagicMock(); e_no_exec.is_undone = False; e_no_exec.event_type="NoExec"; e_no_exec.id=2
    
    e_bad_hydrate = MagicMock(); e_bad_hydrate.is_undone = False; e_bad_hydrate.event_type="BadH"; e_bad_hydrate.id=3
    
    e_cant_undo = MagicMock(); e_cant_undo.is_undone = False; e_cant_undo.event_type="Cant"; e_cant_undo.id=4
    e_cant_undo.event_id = "cant-id"
    
    e_crash = MagicMock(); e_crash.is_undone = False; e_crash.event_type="Crash"; e_crash.id=5
    ...
    def get_executor(etype):
        if etype == "NoExec": return None # Hit 369
        ex = AsyncMock()
        if etype == "Cant": ex.can_undo.return_value = False # Hit 382
        if etype == "Crash": ex.can_undo.side_effect = Exception("UndoFail") # Hit 420
        return ex
    registry.get.side_effect = get_executor
    
    # Hydration logic
    def hydrate(evt):
        if evt.event_type == "BadH": return None # Hit 375
...
async def test_redo_branches():
    ...
    u_bad_orig = MagicMock(); u_bad_orig.causation_id = None; u_bad_orig.id=1 # Hit 523
    u_miss_orig = MagicMock(); u_miss_orig.causation_id = "miss_id"; u_miss_orig.id=2
    u_ok = MagicMock(); u_ok.event_id="u_ok"; u_ok.causation_id = "ok_id"; u_ok.id=3
    
    store.get_events_by_correlation = AsyncMock(return_value=[u_bad_orig, u_miss_orig, u_ok])
    
    # Orig events
    e_ok = MagicMock(); e_ok.is_undone=True; e_ok.event_type="Ok"; e_ok.aggregate_type="A"; e_ok.aggregate_id="1"
    
    async def get_event(eid):
        if eid == "miss_id": return None # Hit 528
        if eid == "ok_id": return e_ok
        return None

...
async def test_autodiscovery():
    ...
    with patch("cqrs_ddd.handler_registry.get_registered_handlers") as mock_get_reg:
        ...
        with patch.object(
            ...
            assert any(
                isinstance(h, AutoPriorityHandler)
                for h in dispatcher._priority_subscribers[UserCreated]
            )

            # AutoBackgroundHandler should be in _subscribers
            assert any(
                isinstance(h, AutoBackgroundHandler)
                for h in dispatcher._subscribers[UserCreated]
            )

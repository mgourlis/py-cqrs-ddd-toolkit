...
class UserRegistrationSaga(Saga[RegistrationState]):
    ...
    async def start(self, initial_data: Any) -> None:
        ...
        self.dispatch_command(
            CreateUser(username=self.state.username, email=initial_data["email"])
        )
...
async def test_orchestration_flow():
    ...
    # choreography_manager = manager  # If we want to simulate unified handling...
    ...
    event = UserCreated(
        user_id="u-123", username="john_doe", correlation_id=correlation_id
    )

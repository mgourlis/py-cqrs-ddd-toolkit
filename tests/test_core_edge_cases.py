...
from cqrs_ddd.core import CommandHandler, QueryHandler, EventHandler, AbstractCommand
    ...
    with patch("inspect.signature", side_effect=ValueError("Boom")):
        with patch("cqrs_ddd.core.logger") as mock_log:
            ...
            class BrokenInspectHandler(CommandHandler):
                async def handle(self, cmd: AbstractCommand):
                    pass

            mock_log.warning.assert_any_call(
                "Error during auto-registration of BrokenInspectHandler: Boom"
            )

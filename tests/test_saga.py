...
class OrderSaga(Saga):
    ...
    async def compensate(self):
        ...
        self.dispatch_command(CancelOrder(order_id=self.context.state.get('order_id', 'unknown')))
...
async def test_saga_failure_compensation():
    ...
    # Start it
    await manager.handle_event(OrderCreated(event_id="e1", correlation_id=correlation_id, order_id="o1"))
    
    # Trigger failure
    fail_evt = PaymentProcessed(event_id="e2", correlation_id=correlation_id, status="fail")

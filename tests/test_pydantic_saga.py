...
from cqrs_ddd.saga import SagaContext, SagaManager, SagaChoreographyManager, InMemorySagaRepository, saga_step
...
async def test_pydantic_saga_start_persistence():
    ...
    saga_id = await manager._process_saga(
        TestOrderSaga, 
        correlation_id, 
...
async def test_pydantic_saga_full_orchestration():
    ...
    event = UserCreated(user_id="u-1", email="test@example.com", correlation_id=correlation_id)

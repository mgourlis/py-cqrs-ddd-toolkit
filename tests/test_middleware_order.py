...
async def test_middleware_execution_order_with_registry():
    ...
    with patch("cqrs_ddd.middleware.middleware.log", side_effect=lambda: lambda cls: cls): # just bypass
...
async def test_validator_middleware_order_and_fail_fast():
    ...
    """
    from cqrs_ddd.middleware import LoggingMiddleware, ValidatorMiddleware, ThreadSafetyMiddleware
    ...
    mock_lock = MagicMock()
    mock_lock.acquire = AsyncMock(side_effect=lambda *a, **k: tracker.record("lock_acquired") or "token")
    mock_lock.release = AsyncMock(side_effect=lambda *a, **k: tracker.record("lock_released"))

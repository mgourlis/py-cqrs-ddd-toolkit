...
import cqrs_ddd.backends.redis_lock as redis_lock_mod
from cqrs_ddd.backends.redis_lock import RedisLockStrategy, InMemoryLockStrategy
...
async def test_in_memory_lock_strategy():
    ...
    # Multi-resource
    token2 = await strategy.acquire("post", ["1", "2"])
    assert f"post:1" in strategy._locks
    assert f"post:2" in strategy._locks
    ...
    # Release
    await strategy.release("user", "123", token)
    assert f"user:123" not in strategy._locks

...
class ApprovalSaga(Saga[HitlState]):
    ...
    async def on_request(self, event: ApprovalRequest):
        ...
        if event.amount > 1000:
            ...
            self.suspend(
                "Large amount requires manual approval", timeout=timedelta(seconds=2)
            )
        else:
...
async def test_hitl_suspend_and_resume():
    ...
    evt1 = ApprovalRequest(
        request_id="r-1", amount=5000.0, correlation_id=correlation_id
    )
    ...
    evt2 = ApprovalReceived(
        request_id="r-1", approved=True, correlation_id=correlation_id
    )
...
async def test_hitl_timeout():
    ...
    evt1 = ApprovalRequest(
        request_id="r-2", amount=5000.0, correlation_id=correlation_id
    )
    await manager.handle_event(evt1)
    ...
    if not ctx.is_failed:
        pytest.fail(
            f"Saga should be failed but is_failed={ctx.is_failed}, suspended={ctx.is_suspended}, state={ctx.state}"
        )

...
class MyOpPersistence(SQLAlchemyOperationPersistence[DummyEntity]):
    ...
    async def persist(self, entity, uow):
        return entity.id
...
async def test_persistence_operations(session_factory):
    ...
    async with uow:
        uow.session.add_all(
            [DummyModel(id="1", name="A"), DummyModel(id="2", name="B")]
        )
...
async def test_persistence_edge_cases(mock_session):
    ...
    class NoModelOp(MyOpPersistence):
        model_class = None
...
async def test_retrieval_query_fallbacks():
    ...
    class FailRetrieval(SQLAlchemyRetrievalPersistence):
        pass
    ...
    class FailQuery(SQLAlchemyQueryPersistence):
        pass
    ...
    class V1Query(SQLAlchemyQueryPersistence):
        dto_class = MockV1

    assert V1Query().to_dto(MagicMock()) == "v1"

    class V1Retrieval(SQLAlchemyRetrievalPersistence):
        entity_class = MockV1
...
async def test_pydantic_v2_read_through():
    ...
    class Retrieval(SQLAlchemyRetrievalPersistence):
    ...
    class Query(SQLAlchemyQueryPersistence):
        dto_class = RawDTO

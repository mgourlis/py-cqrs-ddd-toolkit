...
import cqrs_ddd.backends.storage as storage_mod
from cqrs_ddd.backends.storage import LocalFileSystemStorage, S3Storage, DropboxStorage, GoogleCloudStorage
...
async def test_local_storage_comprehensive(monkeypatch):
    ...
    mock_aio.os.path.exists = AsyncMock(side_effect=[False, True, True, True, True, False, True])
    ...
    with patch("cqrs_ddd.backends.storage.aiofiles.os.listdir", AsyncMock(return_value=["f"])):
...
async def test_s3_storage_comprehensive():
    ...
    mock_client.head_object = AsyncMock(side_effect=[S3ClientError("404"), S3ClientError("500"), {}])
    mock_client.put_object = AsyncMock()
    mock_client.delete_object = AsyncMock()
    
    m_stream = AsyncMock(); m_stream.read.return_value = b"hi"
...
async def test_dropbox_storage_comprehensive():
    mock_client = MagicMock()
    mock_resp = MagicMock()
    mock_resp.status_code = 200; mock_resp.content = b"hi"
...
async def test_gcs_storage_comprehensive():
    ...
    with patch("gcloud.aio.storage.Storage", return_value=AsyncCM(mock_client), create=True):

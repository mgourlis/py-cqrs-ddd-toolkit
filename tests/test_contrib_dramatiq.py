...
def test_setup_dramatiq():
    with patch("cqrs_ddd.contrib.dramatiq_tasks.HAS_DRAMATIQ", True):
        ...
        with patch("cqrs_ddd.contrib.dramatiq_tasks.RabbitmqBroker", create=True) as mock_broker:
            with patch("cqrs_ddd.contrib.dramatiq_tasks.RedisBackend", create=True) as mock_backend:
                with patch("cqrs_ddd.contrib.dramatiq_tasks.Results", create=True):
                    with patch("cqrs_ddd.contrib.dramatiq_tasks.Abort", create=True):
                        with patch("cqrs_ddd.contrib.dramatiq_tasks.Retries", create=True):
                            with patch("cqrs_ddd.contrib.dramatiq_tasks.AsyncIO", create=True):
                                with patch("dramatiq.set_broker", create=True) as mock_set:
                                    ...
                                    mock_broker.assert_called_once_with(url="amqp://test")
...
async def test_handle_dramatiq_event_logic():
    ...
    with patch("cqrs_ddd.event_registry.EventTypeRegistry.hydrate_dict") as mock_hydrate:
